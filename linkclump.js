...
chrome.runtime.sendMessage({
	...
	if (chrome.runtime.lastError) {
		console.log("Linkclump: Runtime error:", chrome.runtime.lastError.message);
		return;
	}
	...
	if (!response.actions) {
		console.log("Linkclump: No actions found in response");
		return;
	}
...
chrome.runtime.onMessage.addListener(function(request, sender, callback) {
	if (request.message === "update") {
		...
		// Service Workerからのクリップボード操作のフォールバック
