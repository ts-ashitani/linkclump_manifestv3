// Service Worker用にsettings_manager.jsをimport
...
// Service Workerではクリップボードアクセスが制限されているため、
// コンテンツスクリプトに委譲する関数
...
function handleRequests(request, sender, callback){
	...
	case "activate":
		...
		case "copy":
			...
			copyToClipboard(text, sender.tab.id);
			break;
		case "bm":
	...
	case "update":
		settingsManager.save(request.settings, function() {
			settingsManager.load(function(settings) {
...
// chrome.extension.onMessage を chrome.runtime.onMessage に変更
chrome.runtime.onMessage.addListener(handleRequests)

settingsManager.isInit(function(isInitialized) {
	if (!isInitialized) {
		...
		settingsManager.isLatest(function(isLatest) {
			if (!isLatest) {
