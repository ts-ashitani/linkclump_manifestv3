...
function check_selection() {
	...
	if (!params || !params.actions) {
		console.log("Options: params or params.actions is undefined in check_selection");
		return;
	}
...
function displayOptions(action) {
	if (!config || !config.actions || !config.actions[action]) {
		console.log("Options: config is undefined in displayOptions");
		return;
	}
...
function save_action(event) {
	if (!params || !params.actions) {
		console.log("Options: params is undefined in save_action");
		return;
	}
	
	if (!config || !config.actions) {
		console.log("Options: config is undefined in save_action");
		return;
	}
...
function save_params() {
	if (!params) {
		console.log("Options: params is undefined in save_params");
		return;
	}
...
$(function() {
	...
	chrome.runtime.sendMessage({
		...
		if (chrome.runtime.lastError) {
			console.log("Options: Runtime error:", chrome.runtime.lastError.message);
			return;
		}
		
		if (!response || response === null) {
			console.log("Options: Unable to load settings due to null response");
			return;
		}
		
		if (!response.actions) {
			console.log("Options: No actions found in response");
			return;
		}
		...
		if (params.blocked && Array.isArray(params.blocked)) {
